    ...
    for det_idx, det in enumerate(det_list):
        ...
        if closest_gt is not None and gt_line_world is not None:
            ...
            metrics.append({
                ...
                'gt_improvement': vma_to_gt - refined_to_gt,
                'pcd_improvement': vma_to_pcd - refined_to_pcd
            })
            
            print(f"样本{count}: VMA→GT={vma_to_gt:.3f}m, Refined→GT={refined_to_gt:.3f}m | VMA→PCD={vma_to_pcd:.3f}m, Refined→PCD={refined_to_pcd:.3f}m")
            count += 1
...
print(f"到GT距离:")
print(f"  VMA平均: {vma_to_gt_avg:.4f}m")
print(f"  Refined平均: {refined_to_gt_avg:.4f}m")
print(f"  改进: {vma_to_gt_avg - refined_to_gt_avg:.4f}m ({(vma_to_gt_avg - refined_to_gt_avg)/vma_to_gt_avg*100:.1f}%)")

print(f"\n到点云距离:")
print(f"  VMA平均: {vma_to_pcd_avg:.4f}m")
print(f"  Refined平均: {refined_to_pcd_avg:.4f}m")
