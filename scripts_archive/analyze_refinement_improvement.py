...
DATA_DIR = "./vma_infer_data_v2"  # 可以改为 "./vma_infer_data" 使用旧数据，或 "./vma_infer_data_v2" 使用新数据
...
def compute_chamfer_distance(pred, gt):
    ...
    if HAS_SCIPY:
        # Pred to GT
        tree_gt = KDTree(gt[:, :2])
        dists_p2g, _ = tree_gt.query(pred[:, :2])
        mean_p2g = np.mean(dists_p2g)
        
        # GT to Pred
        tree_pred = KDTree(pred[:, :2])
        dists_g2p, _ = tree_pred.query(gt[:, :2])
        mean_g2p = np.mean(dists_g2p)
    else:
        # Pure numpy implementation
        # Pred to GT
        diff_p2g = pred[:, :2][:, np.newaxis, :] - gt[:, :2][np.newaxis, :, :]
        dists_p2g = np.min(np.linalg.norm(diff_p2g, axis=2), axis=1)
        mean_p2g = np.mean(dists_p2g)
        
        # GT to Pred
        diff_g2p = gt[:, :2][:, np.newaxis, :] - pred[:, :2][np.newaxis, :, :]
        dists_g2p = np.min(np.linalg.norm(diff_g2p, axis=2), axis=1)
        mean_g2p = np.mean(dists_g2p)
    
    return (mean_p2g + mean_g2p) / 2.0
...
def main():
    ...
    for json_file in json_files:
        ...
        for item in items:
            ...
            try:
                ...
                err_noise = compute_chamfer_distance(noisy_resampled, gt_3d_res)
                err_refine = compute_chamfer_distance(refined_3d, gt_3d_res)
    ...
    print(f"  Mean relative improvement: {relative_improvements.mean():.2f}%")
    print(f"  Median relative improvement: {np.median(relative_improvements):.2f}%")
    ...
    print(f"\nAverage error reduction: {improvements.mean():.4f} m ({relative_improvements.mean():.2f}%)")
    print("="*60)
