...
from projects.mmdet3d_plugin.vma.modules.gram_loss import GramLoss
...
class VMAHead(DETRHead):
    ...
    # 优化 view_map 的图像质量 
    def high_res_smoothing(self, feat):
        """高分辨率平滑：上采样补细节→下采样去噪声，适配view_map低质量场景"""
        B, C, H, W = feat.shape
        # 上采样2倍（bicubic插值，保留细节）
        ...
        # 下采样回原尺寸（过滤噪声）
        feat_smoothed = F.interpolate(
            feat_upsampled, 
            size=(H, W), 
            mode="bicubic", 
            align_corners=False
        )
        return feat_smoothed 
    ...
    def forward(self, 
        ...
        if mlvl_feats_view is not None:
            ...
            for feat_lidar, feat_view in zip(mlvl_feats, mlvl_feats_view):
                ...
                gram_loss += self.gram_loss(feat_view_reshaped, feat_lidar_reshaped)
                # gram_loss = 0  
    ...
    #         mlvl_feats = mlvl_feats_fused
        ...
        for lvl in range(hs.shape[0]):
            ...
            tmp[..., 0:2] += reference[..., 0:2] # 切片操作 reference[:, :, :, 0:2] 
            tmp = tmp.sigmoid() 
    ...
    def loss_single(self,
        ...
        # =================== START: 手动类别权重代码 ===================
        #
        # 手动为 'stop_line' 类别增加权重
        #
        # 1. 定义类别权重, 顺序必须和 map_classes 严格一致:
        #    ['lane_line', 'curb', 'stop_line'] -> 索引 [0, 1, 2]
        cls_weight_values = torch.tensor([1.0, 1.0, 10.0], device=cls_scores.device) # 'stop_line' 权重为 10.0

        # 2. 我们只对前景类别 (positive samples) 应用这个权重
        #    背景类别的索引是 self.num_classes，我们不处理它
        pos_mask = (labels >= 0) & (labels < self.num_classes)

        # 3. 如果存在前景样本
        if pos_mask.any():
            # 3.1. 从 labels 中获取这些前景样本的类别索引
            pos_labels = labels[pos_mask]
            
            # 3.2. 使用这些索引从 cls_weight_values 中取出对应的权重值
            selected_weights = cls_weight_values[pos_labels]
            
            # 3.3. 将选出的权重值乘到原有的 label_weights 上 (只作用于前景样本)
            label_weights[pos_mask] = label_weights[pos_mask] * selected_weights

            # 3.4. 同步增加回归权重 (pts_weights)，让模型更重视停止线的几何形状
            # pts_weights 形状通常为 [N, num_pts, 2] 或类似，需要调整 selected_weights 形状以支持广播
            # selected_weights: [num_pos] -> [num_pos, 1, 1]
            weight_view = selected_weights.view(-1, 1, 1)
            # 确保 pts_weights 和 weight_view 在同一设备上
            if pts_weights.device != weight_view.device:
                weight_view = weight_view.to(pts_weights.device)
            pts_weights[pos_mask] = pts_weights[pos_mask] * weight_view
        # ==================== END: 新增手动类别权重代码 =====================
        ...
        # 下面的 loss_cls 计算会自动使用我们刚刚修改过的、带有类别权重的 label_weights
        loss_cls = self.loss_cls(
            cls_scores, labels, label_weights, avg_factor=cls_avg_factor)
    ...
    def loss(self,
        ...
        if 'gram_loss' in preds_dicts:
            loss_dict['loss_gram'] = preds_dicts['gram_loss']
