...
log_level = 'INFO'
work_dir = "./new_data_train_result/dinov3_later_fusion_spatial"
load_from = None
resume_from = None
workflow = [('train', 1)]
...
model = dict(
    ...
    img_backbone=dict(
        type='DINOv3Backbone',
        model_id="facebook/dinov3-vitb16-pretrain-lvd1689m",  # 选用轻量的ViT-B
        out_channels=256,  # 与Neck的out_channels一致
        frozen_stages=8,  # 冻结前8层，平衡泛化和适配性
        norm_eval=True
    ),

    # 替换 img_backbone_view（view_map 分支）
