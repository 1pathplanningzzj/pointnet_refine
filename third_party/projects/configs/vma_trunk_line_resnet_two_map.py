...
log_level = 'INFO'
work_dir = "./new_data_train_result/resnet_two_map_result_test_2"
load_from = None
resume_from = None
workflow = [('train', 1)]
...
input_modality = dict(
    use_lidar_map=True,
    use_view_map=True,
    use_z_map=True, # npy file should be customed specially
)
# img_keys=["lidar_map"]
...
model = dict(
    ...
    pretrained = None,
    # pretrained=dict(img='ckpts/resnet152-b121ed2d.pth'),  # ckpts/resnet152-b121ed2d.pth   #  ckpts/sd_line.pth
    # pretrained=dict('ckpts/swin_base_patch4_window7_224_22kto1k.pth'),
    img_backbone=dict(
        type='ResNet',
        depth=101, 
        num_stages=4,
    ...
    #     pretrained='ckpts/swin_base_patch4_window7_224_22kto1k.pth',  # 你的预训练权重
    #     embed_dims=128,  # Swin-B 默认基础嵌入维度，保证通道升维逻辑
    #     depths=[2, 2, 18, 2],  # Swin-B 标准层数，无需修改
    #     num_heads=[4, 8, 16, 32],  # 与通道数匹配（4×32=128，8×32=256...）
    #     window_size=7,  # 窗口注意力大小，保持默认
    #     mlp_ratio=4,  # MLP 隐藏层比例，无需修改
    #     qkv_bias=True,  # 启用 QKV 偏置，提升性能
    #     drop_rate=0.1,  # 基础 dropout，保持默认
    #     attn_drop_rate=0.1,  # 注意力 dropout，保持默认
    #     drop_path_rate=0.2,  # 路径 dropout，正则化用
    #     patch_norm=True,  # Patch 归一化，稳定训练
    #     out_indices=(1, 2, 3),  # 输出3个尺度，对应后续升维需求
    #     frozen_stages=1,  # 冻结第1阶段，复用预训练权重
    #     norm_cfg=dict(type='LN', requires_grad=True),  # Swin 必须用 LayerNorm
    #     use_abs_pos_embed=False,  # 已关闭绝对位置嵌入，避免尺寸冲突
    #     act_cfg=dict(type='GELU')  # Swin 标准激活函数
    # ),
    # 主模态 Swin（原配置）
# 修改Swin Backbone配置
    # img_backbone=dict(
    #     type='SwinTransformer',
    #     pretrained='ckpts/swin_base_patch4_window7_224_22kto1k.pth',
    #     embed_dims=128,
    #     depths=[2, 2, 18, 2],
    #     num_heads=[4, 8, 16, 32],
    #     window_size=7,
    ...
    #     frozen_stages=1,
    #     # 关键修改：调整norm配置
    #     norm_cfg=dict(type='LN', requires_grad=True),  # 改为True
    #     # 关键修改：调整位置编码
    #     use_abs_pos_embed=False,  # 改为True
    #     # 关键修改：添加多卡训练相关配置
    #     with_cp=False,  # 禁用checkpoint
    ...
    #     norm_cfg=None),
    # fuse_neck=dict(
    #     type='CrossAttnFeatEnhancer',
    #     in_channels=[512, 1024, 2048],
    #     # offset_channels=64
    #     ),
    view_quality_estimator=dict(
        in_channels=3,  # view_map是3通道RGB
        hidden_dim=64   # 中间层通道数（可调整）
    ),
    fuse_neck=dict(
        type='ImgViewZAlignFuser',
        in_channels=[512, 1024, 2048],  # 与img_backbone的out_indices输出通道一致
        patch_sizes=[[16,16], [8,8], [4,4]],  # 每个阶段的patch尺寸（随分辨率减小而减小）

        # patch_sizes=[[16,16], [8,8], [4,4]],  # 每个阶段的patch尺寸（随分辨率减小而减小）
        num_attn_layers=3,  # 3层交叉注意力
        attn_embed_dims=256,  # 注意力隐藏层维度
        num_heads=8,  # 多头注意力头数
        z_adapter_cfg=dict(
            target_channels=[512, 1024, 2048],
            target_sizes=[(125,125), (63,63), (32,32)]
        )
    ),
    # fuse_neck=dict(
    #     type='CrossAttnFeatEnhancer',
    #     # in_channels与Swin输出通道[256,512,1024]完全对齐
    #     in_channels=[256, 512, 1024],  
    #     # Patch尺寸不变，但此时patch_dim计算会匹配：
    #     # 尺度0：256×16×16=65536，尺度1：512×8×8=32768，尺度2：1024×4×4=16384
...
train_pipeline = [
    ...
    # dict(
    #     type='PointEnhanceLowIntensityTarget',
    #     img_keys=img_keys,
    #     # colormap=cv2.COLORMAP_JET,  # 必须与生成lidar_map时一致
    #     enhance_method="log",  # 或 "power"
    #     weak_thr=0.2,  # 根据强度分布调整
    #     log_gain=15.0
    # ),
    dict(
    ...
    # dict(
    #     type = 'PointEnhanceTOLowIntensityTarget',
    #     img_keys = img_keys,
    # )
    dict(
    ...
    # dict(
    # type='ResizeMultiImage',  # 自定义的多图像统一resize类
    # img_keys=img_keys,
    # size=input_shape  # 强制缩放到1000×1000
    # ),
...
optimizer = dict(
    ...
    paramwise_cfg=dict(
        custom_keys={
            ...
            'fuse_neck': dict(lr_mult=0.3),  # 新增：融合模块降权
        }),
    weight_decay=0.01)
